<script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            inputs.forEach(input => {
                input.addEventListener('input', function() {
                    updatePreview();
                    highlightPreviewSection(this.dataset.preview);
                });
        ...
        function updatePreview() {
            ...
            const descriptionEn = document.getElementById('description_en').value ||
                '{{ __('app.product_description_placeholder') }}';
            const nutritionEn = document.getElementById('nutrition_en').value || '';
            ...
            if (barcodeElement) {
                if (barcode) {
                    barcodeElement.textContent = `{{ __('app.barcode_label') }}: ${barcode}`;
                    barcodeElement.style.display = 'inline-flex';
                } else {
                    barcodeElement.style.display = 'none';
                }
            }

            // Update nutrition info if present
            const nutritionElement = document.getElementById('preview-nutrition');
            if (nutritionElement) {
                if (nutritionEn) {
                    nutritionElement.innerHTML = `<strong>{{ __('app.nutrition_label') }}:</strong> ${nutritionEn}`;
                    nutritionElement.parentElement.style.display = 'block';
                } else {
                    nutritionElement.parentElement.style.display = 'none';
                }
            }
            ...
            if (salePrice && parseFloat(salePrice) < parseFloat(price)) {
                priceElement.innerHTML =
                    `$${salePrice} <small style="text-decoration: line-through; opacity: 0.7;">$${price}</small>`;
            } else {
            ...
            if (isActive) {
                statusElement.innerHTML = '<span class="badge bg-success">{{ __('app.active') }}</span>';
            } else {
                statusElement.innerHTML = '<span class="badge bg-danger">{{ __('app.inactive') }}</span>';
            }
        }

        function highlightPreviewSection(section) {
            if (!section) return;
            ...
            const elementId = elements[section];
            if (elementId) {
                const element = document.getElementById(elementId);
                if (element) {
                    element.classList.add('preview-field-highlight');
                    setTimeout(() => {
                        element.classList.remove('preview-field-highlight');
                    }, 1000);
                }
            }
        }
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    previewImageElement.innerHTML =
                        `<img src="${e.target.result}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 1rem;" alt="Product">`;
        ...
        function removeImage(event) {
            ...
            if (currentImageContainer) {
                ...
                // Keep the existing image in the preview
                @if ($product->image)
                    document.getElementById('preview-image').innerHTML =
                        `<img src="{{ asset('storage/' . $product->image) }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 1rem;" alt="Product">`;
                @else
        ...
        document.getElementById('productForm').addEventListener('submit', function(e) {
            ...
            submitBtn.innerHTML = '<i class="bi bi-hourglass-split me-2"></i>{{ __('app.updating_product') }}';
